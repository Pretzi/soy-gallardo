...
export default function EntriesPageClient() {
  ...
  const setPageAndSlice = useCallback(
    (page: number, nextAllEntries?: Entry[]) => {
          ...
          <div className="mb-4 p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded-lg">
            <div className="flex items-start">
              ...
              <div className="ml-3 flex-1">
                <p className="text-sm font-medium text-yellow-800">🟡 Modo Offline - Trabajando con datos locales</p>
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium text-blue-800">
                    {isSyncing
                      ? '🔄 Sincronizando...'
                      : `⏳ ${pendingCount} ${pendingCount === 1 ? 'entrada pendiente' : 'entradas pendientes'} de sincronización`}
                  </p>
                </div>
              </div>
              {!isSyncing && (
                <Button onClick={syncNow} disabled={isSyncing} className="ml-4 text-sm px-4 py-2">
                  Sincronizar Ahora
                </Button>
              )}
            </div>
                ...
                <div key={entry.id} className="bg-white rounded-lg shadow-md border border-gray-200 p-4">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <OfflineLink
                      ...
                      offlineMessage="Los detalles de entrada no están disponibles sin conexión."
                    >
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {(displayEntries || []).map((entry) => (
                    <tr key={entry.id} className="hover:bg-orange-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{entry.telefono}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{entry.localidad}</td>
            ...
            {!searchQuery && totalCount > itemsPerPage && (
              <div className="mt-6 flex flex-col md:flex-row items-center justify-between gap-4">
                <p className="text-sm text-gray-600">
                  ...
                  {Math.min(currentPage * itemsPerPage, totalCount)} de {totalCount} entradas
                </p>
                  ...
                  {Array.from({ length: Math.ceil(totalCount / itemsPerPage) }, (_, i) => i + 1)
                    ...
                    .map((page, index, array) => (
                      <React.Fragment key={page}>
                        {index > 0 && array[index - 1] !== page - 1 && <span className="px-2 py-2 text-gray-500">...</span>}
                        <Button
